...
The same byte-exact contract applies to `add-docstring`, `add-return-type`, `remove-return-type`, `add-parameter-type`, `remove-parameter-type`, `add-import`, and `add-decorator` rollback paths.
...
`rollback --id` can target any supported logged micro-modification, including parameter-annotation additions and removals, explicit import insertions, explicit decorator insertions, and return-annotation removal, but it still refuses legacy/unverifiable records and project mismatches.
